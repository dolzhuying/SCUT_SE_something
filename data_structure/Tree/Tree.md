...
* **二叉平衡树(AVL)**
  BST在最好的情况下每个节点都有儿子，对于每个操作复杂度为`O(log n)`，但在最坏情况下所有节点只有一边有儿子，每个操作的复杂度退化为`O(n)`，因此希望二叉树能够保持“平衡”，使操作的复杂度保持在`O(n)`，完全二叉树/满二叉树都是AVL树
...
* **伸展树（splay tree）**
  ...
  旋转：之字形：两次方向不同的旋转，从查找结点开始向上走，，一字型：两次方向相同的旋转，从上面两个节点往下旋转把查找结点带上去
