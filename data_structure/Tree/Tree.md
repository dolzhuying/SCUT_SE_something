...
* **二叉树性质**
  ...
  1.任意一棵树，若结点数量为n,则边的数量为`n一1`。
  2.非空二叉树上的叶子结点数等于度为2的结点数加1，即`n=n(2)十1`。
...
* **二叉搜索树（BST）**
  ...
  对于插入操作，如果当前节点为空节点，则在该节点插入，否则比较插入值和当前节点的值的大小，然后递归进入左子树或者右子树

  对于查找操作，如果当前节点为空则不存在，否则比较查找值和当前结点的值的大小，不相等则进入子树，相等则查找成功

  对于删除操作，如果要删除的节点没有儿子，则直接删除，如果有一个儿子，则删除后该节点由该儿子替补上来，如果有两个儿子，则该节点用右子树的最小节点来替换，然后右子树位置需要重新调整
* **二叉平衡树(AVL)**
  BST在最好的情况下每个节点都有儿子，对于每个操作复杂度为`O(log n)`，但在最坏情况下所有节点只有一边有儿子，每个操作的复杂度退化为`O(n)`，因此希望二叉树能够保持“平衡”，使操作的复杂度保持在`O(log n)`，完全二叉树/满二叉树都是AVL树
  ...
  在高度为h的AVL树中，最少节点数`S(h)`由`S(h)=S(h-1)+S(h-2)+1，，S(0)=1,S(1)=2`给出
* **伸展树（splay tree）**

  每一次查找节点后该节点移动至根节点

  希望对每一次旋转操作的复杂度达到`O(log N)`,需要改变双旋策略（单旋转和AVL一致），保证从空树开始任意连续对树操作M次最多花费`O(Mlog N)`时间

  旋转（要访问的节点在两种形状的底部）：之字形：两次方向不同的旋转，从查找结点开始向上走，，一字型：两次方向相同的旋转，从上面两个节点往下旋转把查找结点带上去，，，，，，（如果之字形上来一次之后变成1字型，要先走完原本之字形的两次旋转，而不是变成一字形变化旋转，一字型同理）

  ![image-20241124130335483](assets/image-20241124130335483.png)

  ![image-20241124130417484](assets/image-20241124130417484.png)

  插入操作：正常插入然后移动到根节点，自调整插入：`O(1)`每次操作

  删除操作：移动到根节点然后删除，然后左子树的最大节点移动到根节点，右子树连接到新的根节点
* **B树和B+树**
  ...
  3.每一个叶子节点都包含`k-1`个元素，其中`ceil(m/2)<=k<=m`。
  4.所有的叶子节点都位于同一层。
  5.每个节点中的元素从小到大排列，节点当中任意元素的左子树都小于他，右子树都大于他

  插入操作：先找到对应位置插入，（节点未满就先进入节点，否则进入子树），未溢出无需调整，否则节点内第`ceil(m/2)`个节点上移到父节点，左右元素分裂为两棵子树

  删除操作：删除非叶节点上元素，类似BST，直接前/后驱替换（单儿子找单儿子，双儿子找右子树最小节点）==》转换成删除叶节点元素，，，对于叶节点元素，若删除后未下溢出则无需调整，否则，，，1.跟左右兄弟借：邻兄进入父节点，父亲下来该节点， ，，2.不够借：父下移到左，然后右并过来，注意父节点可能下溢出，同理操作
