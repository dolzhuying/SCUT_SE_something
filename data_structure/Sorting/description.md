...
使用Hibbard增量(`1, 3, 7, . . . , 2k − 1`):最坏`Θ (N3/2)`平均`O(N5/4)`,,,使用Sedgewick增量(`1, 5, 19, 41, 109, . . ., (9 · 4i − 9 · 2i + 1 or 4i − 3 · 2i + 1`):最坏:`O(N4/3)`,平均:`O(N7/6)`
...
1、首先设定一个分界值，通过该分界值将数组分成左右两部分。(分界值选定:2-way,,,median of three)
...
时间复杂度：`O(n*k)`，k为位数，`Θ (r(N + b))`，b是（数字是0-9，字母26），

**10.外排序**

存储数据过大，内存无法全部存储，需要借助外存，假设内存最多承载M个数据，一共有N个，按照M对数据进行分组为`ceil（N/M）`组，每组使用不同文件（k-way 两组每组k个文件，k越大一次写入越多，性能越高），然后按组归并排序，`O(log k ceil(N/M))`
